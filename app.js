...
app.get("/players/",async(request,response)=>{
    ...
    FROM 
        player_details;`;
    const playersArray=await database.all(getAllPlayerDetailsQuery);
    response.send(
        playersArray.map((eachPlayer)=>convertPlayerDetailsDbObjectToResponseObject(eachPlayer))
    );
});
...
app.get("/players/:playerId/matches",async(request,response)=>{
    ...
    WHERE 
        player_id = ${playerId};`;
    const matchesOfArray=await database.all(getAllMatchesOfPlayerQuery);
    response.send(matchesOfArray.map((eachMatch)=>convertMatchDetailsDbObjectToResponseObject(eachMatch))
    );
});
...
app.get("/matches/:matchId/players",async(request,response)=>{
    ...
    WHERE 
        match_id = ${matchId};`;
    const playersOfArray=await database.all(getAllPlayersOfMatchQuery);
    response.send(playersOfArray.map((eachPlayer)=>convertPlayerDetailsDbObjectToResponseObject(eachPlayer))
    );
});
